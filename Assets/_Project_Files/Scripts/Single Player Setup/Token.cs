...
public class Token : MonoBehaviour
{
	...
	[SerializeField] private TokenWaypoints waypoints;
	...
	{
		////Remove this late on
		if (waypoints.DestWaypointIndex + count >= 51) return false;
		...
		void Update ()
		{
			...
			if (SelectionMode) {
				if (Input.GetMouseButtonDown (0)) {
					...
					Vector2 size = new Vector2 (0.85f, 0.85f);

					RaycastHit2D hit = Physics2D.Raycast()//Physics2D.BoxCast (pos, size, 0f, Vector2.zero);
